...
const CountdownTimer = () => {
    ...
    const [muteMain, setMuteMain] = useState(true);
    const [muteRepeater, setMuteRepeater] = useState(true);
    const [hours, setHours] = useState();
    const [minutes, setMinutes] = useState();
    const [seconds, setSeconds] = useState();
    const [showMessage, setShowMessage] = useState(false);
    const [message, setMessage] = useState();
    const [messageDuration, setMessageDuration] = useState(defaultDuration);
    ...
    useEffect(() => {
        let timerIntervals;
        if (timeElapsed === 0 && runTimer) displayMessage('Started', 3000);
    ...
    const displayMessage = (message, duration = defaultDuration) => {
    ...
    return (
        <TimerPage justify="start">
            <HeaderContainer>
                ...
                {showMessage && <Message duration={messageDuration}>{message}</Message>}
            </HeaderContainer>
