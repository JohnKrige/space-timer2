...
const ChainEditItem = (props) => {
    const [showEditItem, setShowEditItem] = useState(false);
    const [h, m, s] = getHMS(props.timeRemaining);

    const editItem = (timeMs, name) => {
        setShowEditItem(false);
        props.editItem(props.index, timeMs, name);
    };

    return (
        <ChainItemContainer isActive={props.isActive}>
            <SetTimerModal onSubmit={editItem} totalTime={props.time} show={showEditItem} onClose={() => setShowEditItem(false)} type="" lessThanHour name={props.name} />
            <ChainItemBackground>
                <NameLabel>
                    <div>{props.name}</div>
                </NameLabel>
