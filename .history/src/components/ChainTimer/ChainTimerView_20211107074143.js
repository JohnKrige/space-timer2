...
const ChainItem = (props) => {
    ...
    const editItem = (timeMs) => {
        ...
        console.log(`You want to edit ${props.index} to time of ${timeMs}`);
        props.editItem(props.index, timeMs);
    };

    return (
        <ChainItemContainer isActive={props.isActive}>
            <SetTimerModal onSubmit={editItem} totalTime={props.time} show={showEditItem} onClose={() => setShowEditItem(false)} type="" lessThanHour />
            <ChainItemBackground>
                <NameLabel>
                    <div>{props.name}</div>
                </NameLabel>
